...
void sortStr(tStr * p, int size)
{
    ...
    for(a=0; a<size-1; a++)
    {
        { if(logF == ((void *)0)) logF = fopen("log.txt","w"); fprintf(logF,"%d", a); close(logF); logF = fopen("log.txt","a"); }
        { if(logF == ((void *)0)) logF = fopen("log.txt","w"); fprintf(logF,"%c", '\n'); close(logF); logF = fopen("log.txt","a"); }
        for(b=a+1; b<size; b++)
        {
