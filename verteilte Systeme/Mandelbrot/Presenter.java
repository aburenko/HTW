...
class Model {
    ...
    private void calcImagePart(int threadNumber) {
        ...
        for (int i=0; i<threadNumber; i++) { 
            CalcImagePart calcPart= new CalcImagePart((int)(((double)i/threadNumber)*h),
                (int)(((double)(i+1)/threadNumber)*h),w,h,this);
            System.out.println("f. " + (int)(((double)i/threadNumber)*h));
        ...
        System.out.println("let repaint" + w + h);
        Picture picture = new Picture(w, h);
        for (int i = 0; i < w; i++) {
            for (int j = 0; j < h; j++) {
                int c = 255;
                
                if(imageArr[i][j] == 1) c = 0;
...
class CalcImagePart implements Runnable {
    ...
    public void run() {
        boolean teser = false;
        if(true) System.out.println("jordan:" + yS + " " + yE);
        for(int i=yS; i<yE; i++) {
            for(int j=0; j<w; j++) {
                if( iter(toMOTO(j,w),toMOTO(i,h))) {
                    m.imageArr[j][i] = 1;
                    teser = true;
                }
                else   
        ...
        if(false) System.out.println("jordan:" + yE);
    }
    // a,b = -1...0...1
